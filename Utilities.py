...
_arrondissements = '2021_2026_Communes_vaudoises_et_arrondissements_Liste_des_communes_avec_districts.csv'
_libresPirates = 'LibresEtPirates.csv'
_partis = 'sd-t-17_02-NRW2023-parteien-appendix.csv'
_sd = "Sans dénom."
...
# _vaud = 7.5366869013  # résultats du PVL dans le Canton
...
#        print('Créé bulletin avec poids {0}, liste {1}, complémentaires {2}, suffrages {3}'.format(self.poids,self.liste,self.complementaires,self.suffrages[0:20]))
...
class Commune():
    ...
    Une commune
    """
    def __init__(self,numero,nom,partis=None):
        self.numero = numero
        self.nom = nom
        self.suffrages = 0
        self.suffrages_par_parti = {}
        if partis :
            for p,v in partis.items():
                if numero in v.total_par_commune.items():                   
                    self.suffrages_par_parti[p] = v.total_par_commune[numero]
                    print("Initialisé {0} avec {1} : {2}".format(nom,p,self.suffrages_par_parti[p]))
                else:
...
#        print("Suffrages du parti    {0}: {1}".format(self.nom, self.suffrages_par_liste))
    ...
    def check(self):
        ...
        # on vérifie la somme des communes
        s1,s2,s4=0,0,0
        for k in self.compacts_par_commune.keys():
            # print("Commune {0}".format(k))
            if isinstance(k, int): # less communes ont un ID numérique
                s1 += self.compacts_par_commune[k]
                s2 += sum(list(self.autres_suffrages_par_commune[k].values()))
                # print(self.nom,self.total_par_commune)
                if k in self.total_par_commune:
                    s4 += self.total_par_commune[k] # pas pour pirates et Libres
                    # if self.compacts_par_commune[k]+sum(list(self.autres_suffrages_par_commune[k].values()))!=self.total_par_commune[k]: print("Attention {6} {0} commune {1} a {2}+{3}!={4} ({5}) - il y a donc des voix supplémentaires por la liste".format(self.nom,k,self.compacts_par_commune[k],sum(list(self.autres_suffrages_par_commune[k].values())),self.total_par_commune[k],self.autres_suffrages_par_commune[k],self.classe))
                 # print("Bon {6} {0} commune {1} a {2}+{3}={4} ({5})".format(self.nom,k,self.compacts_par_commune[k],sum(list(self.autres_suffrages_par_commune[k].values())),self.total_par_commune[k],self.autres_suffrages_par_commune[k],self.classe))
                # print("s1 {0} s2 {1} s4 {2}".format(s1,s2,s4)) 
        # print("{0} {1}: Somme des suffrages par commune {2} + autres {3}, total {4}".format(self.classe,self.nom,s1,s2,s4))
        if not s==s2 :
            print("ERREUR: {0} {1}: Somme des suffrages par commune {2} + autres {3}!={4}, total {5}".format(self.classe,self.nom,s1,s2,s,s4))
            sys.exit()
        """    
...
#            plt.text(-off+v, i - .25, str(v), color='white', ha='right', fontsize=9) # horizontal alignment
    ...
    def candidatsParasites(self,candidats):
        ...
        listes = []
        if self.classe == "Parti":
            listes = [ l.numero for l in self.listes ]
        else:
            listes = [ self.numero ]
...
class Parti(Liste):
    ...
    def miseAjour(self,listes):
        ...
        """
        debug = False
        if debug: print("J'entre dans la miseAjour avec {0} suffrages pour la liste {1} sur des bulletins {2} pour {3}".format(listes[1].autres_suffrages_par_commune[5938][0],0,5938,listes[1].nom))
        for l in self.listes:  # je boucle sur les listes
            for k in l.suffrages_par_liste.keys():   # je boucle encore sur les listes
                ...
                if pi in self.suffrages_par_liste.keys():   # les partis sont indexés par nom
                    if debug: print("{0} Trouvé {1} - j'ajoute {2}".format(self.nom,pi,l.suffrages_par_liste[k]))
                    self.suffrages_par_liste[pi] += l.suffrages_par_liste[k]
                    self.doubles_par_liste[pi] += l.doubles_par_liste[k]
                else:
                    if debug: print("{0} Nouveau {1} - je crée avec {2}".format(self.nom,pi,l.suffrages_par_liste[k]))
                    self.suffrages_par_liste[pi] = l.suffrages_par_liste[k]
                    self.doubles_par_liste[pi] = l.doubles_par_liste[k]
            ...
            for c in l.compacts_par_commune.keys():
                if debug: print("Parti {0} Boucle 2 liste {1} commune {2}".format(self.nom,l.nom,c))
                if c not in self.compacts_par_commune.keys() :  # create first list
                    ...
                    if debug: print("      Boucle 2 je crée compacts {0} et autres {1}".format(l.compacts_par_commune[c],l.autres_suffrages_par_commune[c]))
                else:
                    self.compacts_par_commune[c] += l.compacts_par_commune[c]
                    for k in l.autres_suffrages_par_commune[c].keys():
                        if k in self.autres_suffrages_par_commune[c].keys(): self.autres_suffrages_par_commune[c][k]+=l.autres_suffrages_par_commune[c][k]
                        else: self.autres_suffrages_par_commune[c][k]=l.autres_suffrages_par_commune[c][k]
                        if debug: print("{0}: J'ajoute {1}: {2} pour donner {3} pour commune {4} et bulletin {5}".format(self.nom,l.nom,l.autres_suffrages_par_commune[c][k],self.autres_suffrages_par_commune[c][k],c,k))

                    if debug: print("      Boucle 2 incrémente compacts {0} et autres {1}".format(l.compacts_par_commune[c],l.autres_suffrages_par_commune[c]))
                if debug: print("Boucle 2 de miseAjour avec {0} suffrages pour la liste {1} sur des bulletins {2} pour {3}".format(listes[1].autres_suffrages_par_commune[5938][0],0,5938,listes[1].nom))
...
#        print("Nom {0}".format(nom))
        ...
        for n in nn[:-1]:
            self.nom+=n+" "
        self.nom+=nn[-1]
        # print('Crée candidat {0} liste {1:2}, place {2:2}: {3}'.format(self.numero,self.liste_id,self.place,self.nom))
        self.pvl = None
        self.suffrages_par_liste = {}
        self.doubles_par_liste = {}
...
#        print("Le candidat {0} a des suffrages de {1}".format(self.nom, self.suffrages_par_liste))
    ...
    def communes(self,communes,relatif=True):
        ...
        if relatif:
            ...
            if isCommune:
                deuxPlots("Candidat-{0}-Meilleures-Communes-relatif-a-la-liste".format(goodName(self.nom)))
            else:
            ...
            if isCommune:  # Communes
                ...
                plt.title("{0}: arrondissements".format(self.nom))
                plt.legend(loc="lower right")
                deuxPlots("Candidat-{0}-Arrondissements".format(goodName(self.nom)))

        plt.clf()
            

#############################################################################
def lisBulletins(listes,partis):
    """
...
#    print("Je sors de la lecture avec {0} suffrages pour la liste {1} sur des bulletins {2} pour {3}".format(listes[1].autres_suffrages_par_commune[5938][0],0,5938,listes[1].nom))
    ...
    """
    # distribue les suffrages sur les communes
    # incorrect parce que ça ne tient compte que des suffrages des candidats.
    for k,cd in candidats.items():
        # print("Candidat {0} parti {1} suffrages Lausanne {2}".format(k,cd.liste.parti.nom,sum(list(cd.suffrages_par_commune[5586].values()))))
        for c in cd.suffrages_par_commune.keys():
            communes[c].suffrages_par_parti[cd.liste.parti.nom] += sum(list(cd.suffrages_par_commune[5586].values()))
    """
    
    return 

#############################################################################
def lisArrondissements(communes):
    """
    Attribue les arrondissements aux communes

    Ça ne va pas. Je n'ai pas les Libres 
    """
    arrondissements = {}
    with open(_arrondissements) as f:
        ff = csv.reader(f,delimiter=',')
        for row in ff:
#            print(row)
            if row[0]=='OFS': continue
            c = communes[int(row[0])]
            c.arrondissement = row[2]
            # Crée un arrondissement et initialise les suffrages
            if c.arrondissement not in arrondissements.keys() :
                d = Commune(c.arrondissement,c.arrondissement)
                d.arrondissement = c.arrondissement
                for p in c.suffrages_par_parti.keys():
                    d.suffrages_par_parti[p] = c.suffrages_par_parti[p] 
                arrondissements[c.arrondissement] = d # add to list
            else:
                for p in c.suffrages_par_parti.keys():
                    arrondissements[c.arrondissement].suffrages_par_parti[p] += c.suffrages_par_parti[p]
            arrondissements[c.arrondissement].suffrages += c.suffrages
...
def fixArrondissements(arrondissements,Vaud,partis):
    ...
    Je lis donc le fichier avec les résultats de ces derniers et mets
    à jour leurs résultats
    """
    rows = []
    Vaud.suffrages_par_parti["Libres"] = 0
    Vaud.suffrages_par_parti["Pirates"] = 0
    with open(_libresPirates) as f:
        ff = csv.reader(f,delimiter=',')
        for row in ff: rows.append(row)
        f.close()
    for n,a in arrondissements.items():
        if n not in rows[0]:
            print("Je ne trouve pas {0} dans le fichier des Libres".format(n))
            sys.exit()
        else: idx = rows[0].index(n)
        a.suffrages_par_parti["Libres"] = int(rows[1][idx])
        a.suffrages_par_parti["Pirates"] = int(rows[2][idx])
        a.suffrages_par_parti["Autres"] -= int(rows[1][idx])+int(rows[2][idx])
        Vaud.suffrages_par_parti["Libres"] += int(rows[1][idx])
    ...
    for n,c in communes.items():
        ...
        for cd in candidats.values():
            if a in cd.suffrages_par_commune.keys():
                for k in cd.suffrages_par_commune[n]:
                    if k in cd.suffrages_par_commune[a].keys():
                        cd.suffrages_par_commune[a][k] += cd.suffrages_par_commune[n][k]
                    else:
                        cd.suffrages_par_commune[a][k] = cd.suffrages_par_commune[n][k]
            else:
               cd.suffrages_par_commune[a] = {}
               for k in cd.suffrages_par_commune[n]:
                   cd.suffrages_par_commune[a][k] = cd.suffrages_par_commune[n][k]

    #for a in arrondissements.keys():
    #    for cn in candidats.values(): print(cn.nom,a,cn.suffrages_par_commune[a])
#############################################################################
def lisOFS(partis):
    """
 ...
 #           if row[0]=='Gemeinde' and row[4]=='Vaud':
            if row[4]=='Vaud':
                ...
                if row[5]:
                    num = int(row[5])
               	    if num not in communes.keys():
                        c = Commune(num,nom,partis)
                        communes[num] = c
                else: c = Vaud
                 # initialise
                # Ceci ne connait que les partis OFS. Pas les libres ou les pirates.
                parti = row[9]
    ...
    print("Vaud suffrages par parti {0}".format(Vaud.suffrages_par_parti))
